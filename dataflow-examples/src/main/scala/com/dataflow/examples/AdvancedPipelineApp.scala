...
object AdvancedPipelineApp {
  ...
  private def setupSalesData(): IO[Unit] = IO {
    ...
    val storesData = """{"store_id":"store-nyc","city":"New York","region":"East","manager":"Alice Johnson"}
        ...
        aggregateTransform <- IO {
          new AggregateTransform(
            AggregateConfig(
              ...
              aggregations = Map(
                "total_sales" -> Aggregator.count,
                "total_revenue" -> Aggregator.sum("price"),
                "avg_transaction" -> Aggregator.average("price"),
                "max_sale" -> Aggregator.max("price")
              )
            )
          )
        }
